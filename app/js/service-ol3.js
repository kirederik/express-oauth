 ...
 *  */
inbuServices.factory('Utils', ['Models', 
    function(Models) {
        return {
            /**
             * @doc method
                        ...
                        if (object) {
                            ...
                            $(contents.layers).each(function(index, content) {
                                ...
                                setInterval(function() {
                                    ...
                                    setTimeout(function() { map.addLayer(layer); }, 10);
                                }, 60000);
                            });
                            map.getView().setCenter([contents.map.centerLon,  contents.map.centerLat]); 
                            map.getView().setZoom(contents.map.zoom);
                            // Utils.center(map, contents.map.centerLon, contents.map.centerLat, contents.map.zoom);
                        } else {
                            if (map.getLayers().length) {
                                map.getView().getView2D().setCenter(ol.projection.transform(
                                    new ol.Coordinate(-48, -20), 'EPSG:4326', map.getView().getView2D().projection)
                                );
                            }
                        }
