...
/**
 * @ngdoc overview
 * @name inbu.directives
 * @description
 */
var inbuDirectives = angular.module('inbu.directives', []);


/**
 * @ngdoc directive
 * @name inbu.directives:map
 * @description A Map
 * @link inbu.services:MapObject
 */
inbuDirectives.directive('map', ['$rootScope', 'MapObject', 'Rest', 'Utils'
...
/**
 * @ngdoc directive
 * @name inbu.directives:natural
 * @description Natural number validator
 */
inbuDirectives.directive('natural', function() {
  return {
        require: 'ngModel',
        link: function(scope, elm, attrs, ctrl) {
            ...
            ctrl.$formatters.push(function(viewValue) {
                ...
                if (INTEGER_REGEXP.test(viewValue) && (vvalue > max && vvalue < min)) {
                    ctrl.$setValidity('natural', true);
                    return viewValue;
                } else {
                    ctrl.$setValidity('natural', false);
...
/**
 * @ngdoc directive
 * @name inbu.directives:buildSlide
 * @description Build the slide based on a json description
 */
 inbuDirectives.directive('buildSlide', ['$compile', 'Builder'
...
/**
 * @ngdoc directive
 * @name inbu.directives:slideBuilder
 * @description Slide selector. Contains a button that creates a new slide-tile.
 *
 *   PT-BR: Seletor de slide. Contêm um botão que cria um novo slideTile.
 *          Diretiva responsável por compilar o SlideTile
 * @link slideTile
 */
inbuDirectives.directive('slideBuilder', ['$compile', function ($compile) {
    return {
        restrict: 'A',
        scope: { text: '@' },
        replace: true,
        template: '<a href="" class="slideBuilder" data-ng-click="newSlide()"><i class="icon-plus"></i>{{ text }}</a>',
        link: function (scope, element, attrs, ctrl) {
...
/**
 * @ngdoc directive
 * @name inbu.directives:slideTile
 * @description A slide tile that can be selected on the left menu. Compiles a new slide.
 *              On click in this directive, change the slide active in the slides area.
 *
 *   PT-BR: Cria o slideTile que pode ser selecionado. Este slideTile está contido no
 *             slideBuilder. É responsável por compilar o slide.
 *          Ao se clicar nesta diretiva, muda o slide ativo na área dos slides.
 *
 * @link slide slideBuilder
 */
inbuDirectives.directive('slideTile', ['$rootScope', '$compile'
    , function($rootScope, $compile){
        return {
            ...
            , link: function(scope, element, attrs, ctrl) {
                ...
                var slide = $compile("<slide parent='" + attrs.id + "' id='slide" + attrs.id + "'></slide>")(scope);
                $(".fill").append(slide);
            }
...
/**
 * @ngdoc directive
 * @name inbu.directives:tile
 * @description A Tile is a widget, its the frame that actually contains the content of a slide
 *              A Slide is composed by one or more tiles
 * @link slide
 */
inbuDirectives.directive('tile', ['$rootScope', '$compile'
    , function($rootScope, $compile){
        return {
            restrict: "E",
            replace: true,
            ...
            template: '<div id="" data-ng-click="select($event)" class="tile selected border-color-darken"> \
                <div class="drag-handle"> <i class="icon-move fg-color-white"></i></div> \
            </div>'
            , link: function(scope, element, attrs, ctrl) {
                // When the tile is created, give a random color to it
                var colors =  [
                    "bg-color-green", "bg-color-greenDark", "bg-color-greenLight",
                    "bg-color-magenta", "bg-color-pink", "bg-color-pinkDark",
                ...
                // Flag to control the drag effect. Used to keep the "selected" class when the user drag the tile
                scope.dragged = false;
...
/**
 * @ngdoc directive
 * @name inbu.directives:presentation
 * @description Control the slides
 */
inbuDirectives.directive('presentation', [ function(){
            ...
            $scope.ready = function(pres) {
                $scope.$watch('nready', function(ov, nv) {
                    ...
                    angular.forEach(pres, function(d, index) {
                        // console.log('{slide: ', d.slide.id, ', time: ', d.slide.time, '}' );
...
inbuDirectives.directive('slideControl', [ function(){
    return {
        restrict: 'A',
        link: {
            post: function($scope, element, iAttrs, controller) {
...
/**
 * @ngdoc directive
 * @name inbu.directives:slide
 * @description A slide is an interface that has the tiles.
 *              A presentation contains several slides
 * @childOf slideTile
 * @link tile
 */
inbuDirectives.directive('slide', ['$rootScope', '$compile', 'Builder', 'Models', 'Utils'
    ...
    return {
        ...
        link: function(scope, elem, attrs, contrl) {
            ...
            scope.toggleForm = function(event) {
                ...
                if (scope.newWidForm) {
                    var newTile = $compile(
                        "<tile parent='" + attrs.id + "' id='tile" + attrs.id + scope.tilesCounter++ + "'></tile>"
                    )(scope);
        ...
        controller: function($scope, $element, $attrs) {
            ...
            /*
             * @ngdoc method
             * @methodOf inbu.directives:slide
             * @name inbu.directives:slide#toggleLayer
